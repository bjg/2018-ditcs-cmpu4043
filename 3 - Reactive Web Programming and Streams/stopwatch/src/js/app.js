...
function drawClock(timeValue) {
    ...
    function outerCirc() {
        ctx.beginPath();
        ctx.arc(canvas.width / 2, canvas.height / 2, widthSize + 7, 0, Math.PI * 2);
    ...
    secondMarks();
    hourMarks();
    secondHand(timeValue);
    minuteHand(timeValue);
...
function showSplit(t){
    if(init){
        if(splitMax<5){
            ...
            splitMax++;
            splitList.innerHTML+="<br> MAXIMUM 5 SPLITS ALLOWED";
        }
    }
}
