...
/* harmony import */ var _css_calculator_css__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(201);
/* harmony import */ var _css_calculator_css__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_css_calculator_css__WEBPACK_IMPORTED_MODULE_2__);
...
var operator = ''; // most recent operator

var equation = []; // the equation to be calculated

var result = ''; // result of calculation

var error_flag = false; // check for errors in calculation

var operators = '+,-,*,x,/,÷';
var numbers = '0123456789';
var symbols = '().';
var last_sym = ''; // main display/input element

var input = document.getElementById("input"); // arrays to store the various buttons

var btns = document.getElementsByClassName("btn"); // create two streams for clicks and keypresses and merge them

var input$ = Object(rxjs__WEBPACK_IMPORTED_MODULE_1__["merge"])(Object(rxjs__WEBPACK_IMPORTED_MODULE_1__["fromEvent"])(btns, 'click').pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_0__["pluck"])('target', 'innerHTML')), Object(rxjs__WEBPACK_IMPORTED_MODULE_1__["fromEvent"])(document, 'keypress').pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_0__["pluck"])('key'))); // subscribe to the stream to observe inputs
...
function displayInput(symbol) {
  ...
  if (error_flag) {
    ...
    var number = symbol * 1; // check if number is integer
...
function clearDisplay() {
  // clear the input box
  input.innerHTML = ''; // clear global variables
...
function equals() {
  ...
  var eq = equation.join(''); // handle the expected values for multiply and divide

  eq = eq.replace('x', '*');
  eq = eq.replace('÷', '/'); // to store the result of the calculated equation
  ...
  try // try calculate the equation
  {
    ...
    float_result = eval(eq); // check result for float values
    ...
    input.innerHTML = result; // reset the arrays

    equation = []; // store the result in the equation to allow for further calculations

    equation.push(result);
  } catch (err) // catch any weird stuff
  {
    // give feedback
    input.innerHTML = "ERROR"; // clear the global variables

    clearVariables(); // set the error flag
  ...
  if (operator === '') {
    ...
    for (var j = 0; j < equation.length; j++) {
      ...
      if (Number.isInteger(equation[j])) {
        console.log("INSIDE check for integer: " + equation[0]); // switch the integer's sign to positive/negative

        equation[j] = equation[j] * -1; // update the display

        input.innerHTML = equation.join(''); // number is found, dont search any further
  ...
  } else // this is not the first number
    {
      ...
      for (var i = equation.length - 1; i > 0; i--) {
        ...
        if (equation[i] === operator) {
          ...
          for (var k = i + 1; k < equation.length; k++) {
            ...
            if (Number.isInteger(equation[k])) {
              ...
              equation[k] = equation[k] * -1; // handle a negative Integer when the operator is also minus

              if (equation[i] === '-') {
                // switch the integer's sign back to positive
                equation[k] = equation[k] * -1; // change the operator to a plus

                equation[i] = '+'; // update the global operator

                operator = '+';
              } // for switching the Integer back to negative
              else if (equation[i] === '+') {
                  // switch the integer's sign back to negative
                  equation[k] = equation[k] * -1; // change the operator to a minus
