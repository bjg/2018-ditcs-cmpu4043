...
const validateTypedInput = (inputVal, inputType) => {
    ...
    }else inputChracter = inputVal[inputVal.length - 1]
    ...
    if(pattern.includes(inputChracter)){
        if (inputChracter === '='){
            ...
            if (!success) input.value = input.value.slice(0, -1);
        }
...
merge(
    fromEvent(document.getElementsByTagName('button'), "click").pipe(map( e => {
        return {
            type: 'button',
    ...
    fromEvent(input, 'input').pipe(map( e => {
        return {
            type: 'input',
            value: e.data,
