...
const is_new_display = () => display_value.value === "0" || display_is_answer;
...
const last_num_in_display = () => display_value.value.match(/(-)?(\+)?\d+(\.)?(\d+)?(?=\D*$)/g)[0];
...
const end_of_display = () => display_value.value.charAt(display_value.value.length-1);


//called when amendments made to display_value.value. ensures new string value is not longer than display screen (if it is then display error), else updates innerHTML of the display div
const update_display = () => {
  var display = document.querySelector("#display");
  var display_text = document.querySelector("#display-text");
  display_text.innerHTML = display_value.value;
  display_text.innerHTML = wider_than(display_text, display) ? display_value.value ="error" : display_value.value;
}//end update_display()
...
const handle_sign_operator = (val) => {
  ...
  switch(val) {
    ...
    case "+":
      display_value.value = display_value.value.replace(end_of_display(), '-');
      break;
    ...
    case '0':
      ...
      last_num_in_display().length === 1 ? display_value.value = "-" : handle_sign_operator(last_num_in_display());
      break;
    //else if last char of display is a digit, change sign of last number on display
    default:
      if(is_digit(end_of_display())) {
        //convert last number in display string to float, multiply it by -1, convert back to String, replace last number in display_value.value with this value
        new_val = parseFloat(last_num_in_display()) * -1;
        new_val = new_val > 0 ? "+"+String(new_val) : String(new_val);
        display_value.value = display_value.value.replace(last_num_in_display(), new_val);
      }
      break;
  }//end switch()
...
const append_to_display = (value) => {
  ...
  else if(is_operator(value) && is_operator(end_of_display())) 
  {
    ...
    display_value.value = display_value.value.replace(end_of_display(), value);
  }
  ...
  else if(display_value.value == "error" || (is_new_display() && is_digit(value))) 
  {
...
//used to add closing brackets to display_value.value where nessecary
...
//parses display_value.value, checks for errors and if none, evaluates expression, updates display
const solve = (none) => {
  display_value.value= format_equation(display_value.value);
  if(display_value.value !== "error") {
    //convert to string to compute length
    display_value.value = String(eval(display_value.value));
    //handle long decimal or recurring decimal answers e.g 1/9 (ref: line 33 & 34)
    if(display_value.value.length > 10 && contains_decimal(display_value.value)) {
          display_value.value = format_decimal_answer(display_value.value);
    }
  }
...
buttons.forEach(el => {
  if(el.id === "clear") {
    observables.push(Observable.fromEvent(el, 'click').mapTo(clear));
  }
  else if(el.id === "equals") {
    observables.push(Observable.fromEvent(el, 'click').mapTo(solve));
  }   
  else {   
    observables.push(Observable.fromEvent(el, 'click').mapTo(append_to_display(el.querySelector('span').innerHTML)));
  }
});

const buttons$ = Observable.merge(observables);
buttons$
  .scan( (acc, update) => update(acc), display_value)*/
    ...
    if(event.key === '-' && event.altKey) {
      ...
      switch(event.key){
        ...
        case 'Enter':
          ...
          break;
        case 'Backspace':
          display_value.value = display_value.value.slice(0,display_value.value.length-1);
          update_display();
          break;
        default:
          break;
